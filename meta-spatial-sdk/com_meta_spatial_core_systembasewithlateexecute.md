# SystemBaseWithLateExecute Class
...
Base class for systems that require a lateExecute call that is run after all systems have run their execute call. lateExecute calls of a system are not guaranteed to run before or after any other system's lateExecute call. If you do not need a lateExecute call, use SystemBase instead.

## Signature

```kotlin
abstract class SystemBaseWithLateExecute : SystemBase
```

## Constructors

| **SystemBaseWithLateExecute** () | Signature ```kotlin constructor() ``` Returns [SystemBaseWithLateExecute](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_systembasewithlateexecute) |
| --- | --- |
...
| **execute** () | System should perform all the operations based on relevant entities. Signature ```kotlin abstract fun execute() ``` |
...
| **lateExecute** () | Called after all systems have executed their execute function. Signature ```kotlin abstract fun lateExecute() ``` |