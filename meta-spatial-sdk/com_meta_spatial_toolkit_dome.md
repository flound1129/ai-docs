...
Defines the dimensions of a dome shape by a radius. This is to be used with the `mesh://dome` URI on [Mesh](/reference/spatial-sdk/v0.10.1/com_meta_spatial_toolkit_mesh) component. Material can be customized with the [Material](/reference/spatial-sdk/v0.10.1/com_meta_spatial_toolkit_material) component.

Domes are commonly used for skyboxes and environmental backgrounds in 3D scenes.
...
```kotlin Entity.create(
    listOf(
        Mesh(Dome.URI),
        Dome(950f), // 950 is ~1km radius which is the default clip distance
        Material().apply {
          baseColor = Color4(red = 0.5f, green = 0.7f, blue = 1.0f, alpha = 1.0f)
        },
...
| **Dome** ( radius ) | Signature ```kotlin constructor(radius: Float = 950.0f) ``` Parameters radius: Float The radius of the dome in meters Returns [Dome](/reference/spatial-sdk/v0.10.1/com_meta_spatial_toolkit_dome) |
| --- | --- |
...
| **radius** : Float [Get][Set] | The radius of the dome in meters Signature ```kotlin var radius: Float ``` |
...
| **createDefaultInstance** : Function0 [Get] | Signature ```kotlin open override val createDefaultInstance: () -> Dome ``` |
...
| **radiusData** : [FloatAttributeData](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_floatattributedata) [Get] | Signature ```kotlin val radiusData: FloatAttributeData ``` |
| **radiusId** [Get] | Signature ```kotlin val radiusId: ``` |
| **URI** : Uri [Get] | URI for the procedural dome mesh. Use with [Mesh](/reference/spatial-sdk/v0.10.1/com_meta_spatial_toolkit_mesh) component. Signature ```kotlin val URI: Uri ``` |
