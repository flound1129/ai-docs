...
IsdkCurvedPanel If added to an entity that has an IsdkPanel, this will make the panel curved.

## Signature

```kotlin
class IsdkCurvedPanel(fieldOfView: Float = 90.0f) : ComponentBase
```

## Constructors

| **IsdkCurvedPanel** ( fieldOfView ) | Signature ```kotlin constructor(fieldOfView: Float = 90.0f) ``` Parameters fieldOfView: Float in degrees for the curved panel Returns [IsdkCurvedPanel](/reference/spatial-sdk/v0.10.1/com_meta_spatial_isdk_isdkcurvedpanel) |
| --- | --- |
...
| **fieldOfView** : Float [Get][Set] | field of view degrees for the curved panel. If less or equal to 0.0, the panel will be treated as flat. Signature ```kotlin var fieldOfView: Float ``` |
...
| **createDefaultInstance** : Function0 [Get] | Signature ```kotlin open override val createDefaultInstance: () -> IsdkCurvedPanel ``` |
| **enumClassesMap_** : Map [Get] | Signature ```kotlin val enumClassesMap_: Map<Int, Class<out Enum<*>>> ``` |
| **fieldOfViewData** : [FloatAttributeData](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_floatattributedata) [Get] | Signature ```kotlin val fieldOfViewData: FloatAttributeData ``` |
| **fieldOfViewId** [Get] | Signature ```kotlin val fieldOfViewId: ``` |
