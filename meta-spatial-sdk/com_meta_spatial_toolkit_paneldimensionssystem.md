...
Setting the `PanelDimensions` component on an entity will update these overrides.
...
| **PanelDimensionsSystem** () | Signature ```kotlin constructor() ``` Returns [PanelDimensionsSystem](/reference/spatial-sdk/v0.10.1/com_meta_spatial_toolkit_paneldimensionssystem) |
| --- | --- |
...
| **getDependencies** () | This system must run after the PanelCreationSystem to ensure that the PanelDimensions are set Signature ```kotlin open override fun getDependencies(): SystemDependencies? ``` Returns [SystemDependencies?](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_systemdependencies) |
