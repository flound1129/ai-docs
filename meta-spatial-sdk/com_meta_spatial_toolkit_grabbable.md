...
Grabbable is a component that allows an object to be grabbed by a controller. It requires the Entity to have a [SceneObject](/reference/spatial-sdk/v0.10.1/com_meta_spatial_runtime_sceneobject) attached.
...
| **Grabbable** ( enabled , type , isGrabbed , minHeight , maxHeight ) | Signature ```kotlin constructor(enabled: Boolean = true, type: GrabbableType = GrabbableType.FACE, isGrabbed: Boolean = false, minHeight: Float = -Float.MAX_VALUE, maxHeight: Float = Float.MAX_VALUE) ``` Parameters enabled: Boolean Defines whether the object can be grabbed or not. type: [GrabbableType](/reference/spatial-sdk/v0.10.1/com_meta_spatial_toolkit_grabbabletype) The type of behavior an object has when grabbed (faces user, pivots on y axis, etc.) isGrabbed: Boolean Whether the object is currently grabbed or not minHeight: Float the minimum height an object can be held when grabbed maxHeight: Float the maximum height an object can be held when grabbed Returns [Grabbable](/reference/spatial-sdk/v0.10.1/com_meta_spatial_toolkit_grabbable) |
| --- | --- |
...
| --- | --- |
| **enabled** : Boolean [Get][Set] | Defines whether the object can be grabbed or not. Signature ```kotlin var enabled: Boolean ``` |
| **entID** : Long [Get][Set] | Signature ```kotlin var entID: Long ``` |
| **isDirty** : Boolean [Get][Set] | Signature ```kotlin var isDirty: Boolean ``` |
| **isGrabbed** : Boolean [Get][Set] | Whether the object is currently grabbed or not Signature ```kotlin var isGrabbed: Boolean ``` |
| **maxHeight** : Float [Get][Set] | the maximum height an object can be held when grabbed Signature ```kotlin var maxHeight: Float ``` |
| **minHeight** : Float [Get][Set] | the minimum height an object can be held when grabbed Signature ```kotlin var minHeight: Float ``` |
| **recycled** : Boolean [Get][Set] | Signature ```kotlin var recycled: Boolean ``` |
| **timeStamp** : Long [Get][Set] | Signature ```kotlin var timeStamp: Long ``` |
| **type** : [GrabbableType](/reference/spatial-sdk/v0.10.1/com_meta_spatial_toolkit_grabbabletype) [Get][Set] | The type of behavior an object has when grabbed (faces user, pivots on y axis, etc.) Signature ```kotlin var type: GrabbableType ``` |
...
| **createDefaultInstance** : Function0 [Get] | Signature ```kotlin open override val createDefaultInstance: () -> Grabbable ``` |
...
| **isGrabbedData** : [BooleanAttributeData](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_booleanattributedata) [Get] | Signature ```kotlin val isGrabbedData: BooleanAttributeData ``` |
| **isGrabbedId** [Get] | Signature ```kotlin val isGrabbedId: ``` |
| **keyStringToKeyIntMap_** : Map [Get] | Signature ```kotlin val keyStringToKeyIntMap_: Map<String, Int> ``` |
| **maxHeightData** : [FloatAttributeData](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_floatattributedata) [Get] | Signature ```kotlin val maxHeightData: FloatAttributeData ``` |
| **maxHeightId** [Get] | Signature ```kotlin val maxHeightId: ``` |
| **minHeightData** : [FloatAttributeData](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_floatattributedata) [Get] | Signature ```kotlin val minHeightData: FloatAttributeData ``` |
| **minHeightId** [Get] | Signature ```kotlin val minHeightId: ``` |
