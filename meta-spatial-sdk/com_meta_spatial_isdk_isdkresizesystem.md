...
IsdkResizeSystem handles panel resize interactions based on IsdkPanelResize component configuration.

This system listens for pointer events with SemanticType.Resize and implements:

- Simple mode: Resize modifies entity scale component - Relayout mode: Resize adjusts panel dimensions and re-renders UI - None mode: Resize handles do nothing. Developers can write their own resize logic via InputListeners on the PanelSceneObject.

The system manages resize state, applies constraints, and updates panel dimensions or scale accordingly.

## Signature

```kotlin
class IsdkResizeSystem : SystemBase
```

## Constructors

| **IsdkResizeSystem** () | Signature ```kotlin constructor() ``` Returns [IsdkResizeSystem](/reference/spatial-sdk/v0.10.1/com_meta_spatial_isdk_isdkresizesystem) |
| --- | --- |
...
| --- | --- |
| **delete** ( entity ) | Clean up when entity is deleted Signature ```kotlin open override fun delete(entity: Entity) ``` Parameters entity: [Entity](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_entity) |
