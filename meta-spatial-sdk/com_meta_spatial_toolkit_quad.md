...
Defines a single-sided 2D rectangular shape in 3D space by specifying minimum and maximum coordinates. This is to be used with the `mesh://quad` URI on [Mesh](/reference/spatial-sdk/v0.10.1/com_meta_spatial_toolkit_mesh) component. Material can be customized with the [Material](/reference/spatial-sdk/v0.10.1/com_meta_spatial_toolkit_material) component.

Quads are commonly used for flat surfaces like panels, screens, or simple rectangular objects.
...
```kotlin Entity.create(
    listOf(
        Mesh(Quad.URI),
        Quad(Vector2(0.1f, 0.1f), Vector2(0.8f, 0.4f)), // Min and max coordinates
        Material().apply {
          baseColor = Color4(red = 1.0f, green = 0.0f, blue = 0.1f, alpha = 1.0f)
        },
...
| **Quad** ( min , max ) | Signature ```kotlin constructor(min: Vector2 = Vector2(0.0f, 0.0f), max: Vector2 = Vector2(1.0f, 1.0f)) ``` Parameters min: [Vector2](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_vector2) The minimum coordinates (bottom-left) of the quad max: [Vector2](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_vector2) The maximum coordinates (top-right) of the quad Returns [Quad](/reference/spatial-sdk/v0.10.1/com_meta_spatial_toolkit_quad) |
| --- | --- |
...
| **max** : [Vector2](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_vector2) [Get][Set] | The maximum coordinates (top-right) in meters of the quad Signature ```kotlin var max: Vector2 ``` |
| **min** : [Vector2](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_vector2) [Get][Set] | The minimum coordinates (bottom-left) in meters of the quad Signature ```kotlin var min: Vector2 ``` |
...
| **createDefaultInstance** : Function0 [Get] | Signature ```kotlin open override val createDefaultInstance: () -> Quad ``` |
...
| **URI** : Uri [Get] | URI for the procedural quad mesh. Use with [Mesh](/reference/spatial-sdk/v0.10.1/com_meta_spatial_toolkit_mesh) component. Signature ```kotlin val URI: Uri ``` |
