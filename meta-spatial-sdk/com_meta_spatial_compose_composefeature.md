...
```kotlin
class ComposeFeature : SpatialFeature
```

## Constructors

| **ComposeFeature** () | Signature ```kotlin constructor() ``` Returns [ComposeFeature](/reference/spatial-sdk/v0.10.1/com_meta_spatial_compose_composefeature) |
| --- | --- |

## Properties

| **panelViewLifecycleOwner** : [PanelViewLifecycleOwner](/reference/spatial-sdk/v0.10.1/com_meta_spatial_compose_panelviewlifecycleowner) [Get] | Signature ```kotlin val panelViewLifecycleOwner: PanelViewLifecycleOwner ``` |
| --- | --- |
...
| **onDestroy** () | Called when the application is in onDestroy callback. Be aware that onDestroy() is not guaranteed to be called. Override this method to perform actions during the OnDestroy() lifecycle of the application. Signature ```kotlin open override fun onDestroy() ``` |
...
| **onStart** () | Called when the application is started. Override this method to perform actions during the OnStart() lifecycle of the application. Signature ```kotlin open override fun onStart() ``` |
| **onStopActivity** () | Called when the activity is in onStop callback. Be aware that onStop() is not guaranteed to be called. Override this method to perform actions during the OnStop() lifecycle of the application. Signature ```kotlin open override fun onStopActivity() ``` |
