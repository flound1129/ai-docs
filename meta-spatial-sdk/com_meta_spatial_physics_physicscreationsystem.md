...
System responsible for creating and managing physics objects. Monitors changes to Physics components and loads collision shapes from local or networked assets. Handles both HTTP/HTTPS URLs and local file paths for collision shapes.

## Signature

```kotlin
class PhysicsCreationSystem(val physicsBridge: PhysicsBridge) : SystemBase
```

## Constructors

| **PhysicsCreationSystem** ( physicsBridge ) | Signature ```kotlin constructor(physicsBridge: PhysicsBridge) ``` Parameters physicsBridge: [PhysicsBridge](/reference/spatial-sdk/v0.10.1/com_meta_spatial_physics_physicsbridge) The PhysicsBridge used to create and delete physics objects Returns [PhysicsCreationSystem](/reference/spatial-sdk/v0.10.1/com_meta_spatial_physics_physicscreationsystem) |
| --- | --- |
...
| --- | --- |
| **physicsBridge** : [PhysicsBridge](/reference/spatial-sdk/v0.10.1/com_meta_spatial_physics_physicsbridge) [Get] | The PhysicsBridge used to create and delete physics objects Signature ```kotlin val physicsBridge: PhysicsBridge ``` |
...
| --- | --- |
| **delete** ( entity ) | System should do any housekeeping based on [PhysicsCreationSystem.delete](/reference/spatial-sdk/v0.10.1/com_meta_spatial_physics_physicscreationsystem#delete) being removed from the scene Signature ```kotlin open override fun delete(entity: Entity) ``` Parameters entity: [Entity](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_entity) |
