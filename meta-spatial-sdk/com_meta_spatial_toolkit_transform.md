...
A component that represents a transform, which is the position and orientation of an entity. The transform is represented by a [Pose](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_pose) , which is a combination of a position ( [Vector3](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_vector3) ) and an orientation ( [Quaternion](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_quaternion) ).

Here is an example  of using a Transform component:

```kotlin val transform = myEntity.getComponent<Transform>()
val position = transform.transform.t
val rotation = transform.transform.q
position.y = position.y + 1f // Move the entity up by 1 meter
myEntity.setComponent(transform) ```

You can build a Transform using the [TransformBuilder](/reference/spatial-sdk/v0.10.1/com_meta_spatial_toolkit_transformbuilder) class:

```kotlin // Make transform that moved up 1 meter and is rotated 90 degrees clockwise around the y-axis.
val transform = Transform(TransformBuilder().move(0f, 1f, 0f).then(TransformBuilder().rotateY(90f)).build()) ```
...
| **Transform** ( transform ) | Signature ```kotlin constructor(transform: Pose = Pose()) ``` Parameters transform: [Pose](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_pose) Returns [Transform](/reference/spatial-sdk/v0.10.1/com_meta_spatial_toolkit_transform) |
| --- | --- |
...
| **transform** : [Pose](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_pose) [Get][Set] | The transform, which identifies the position and orientation of the entity. Signature ```kotlin var transform: Pose ``` |
...
| **createDefaultInstance** : Function0 [Get] | Signature ```kotlin open override val createDefaultInstance: () -> Transform ``` |
...
| **transformData** : [PoseAttributeData](/reference/spatial-sdk/v0.10.1/com_meta_spatial_core_poseattributedata) [Get] | Signature ```kotlin val transformData: PoseAttributeData ``` |
| **transformId** [Get] | Signature ```kotlin val transformId: ``` |
...
| **build** ( init ) | Signature ```kotlin fun build(init: TransformBuilder.() -> Unit): Transform ``` Parameters init: Function1 Returns [Transform](/reference/spatial-sdk/v0.10.1/com_meta_spatial_toolkit_transform) |
