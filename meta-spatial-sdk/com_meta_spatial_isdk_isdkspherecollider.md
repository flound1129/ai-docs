...
IsdkSphereCollider provides a sphere for collision detection in ISDK interactions.

## Signature

```kotlin
class IsdkSphereCollider(radius: Float = 0.05f) : ComponentBase
```

## Constructors

| **IsdkSphereCollider** ( radius ) | Signature ```kotlin constructor(radius: Float = 0.05f) ``` Parameters radius: Float of the spherical collider Returns [IsdkSphereCollider](/reference/spatial-sdk/v0.10.1/com_meta_spatial_isdk_isdkspherecollider) |
| --- | --- |
...
| **radius** : Float [Get][Set] | radius of the spherical collider Signature ```kotlin var radius: Float ``` |
...
| **createDefaultInstance** : Function0 [Get] | Signature ```kotlin open override val createDefaultInstance: () -> IsdkSphereCollider ``` |
